...
class Vehicles extends ConnectionCreator
{
  ...
  {
    $stmt = $this->conn->query("SELECT a.*, b.name AS color_name FROM vehicles AS a INNER JOIN color AS b ON a.color_id = b.id WHERE a.id = {$vehicle_id} ORDER BY a.id DESC");
    $stmt->execute();
  ...
  {
    $stmt = $this->conn->prepare("UPDATE vehicles SET owner_name = ?, color_id = ?, model = ?, plate = ?, updated_at = ? WHERE id = ?;");
    $stmt->bindValue(1, $vehicle['owner_name']);
    $stmt->bindValue(2, $vehicle['color_id']);
    $stmt->bindValue(3, $vehicle['model']);
    $stmt->bindValue(4, $vehicle['plate']);
